...
class LanguageMeta(type, metaclass=LanguageUberMeta):
    ...
    def __getitem__(cls, item):
        ...
        if ' ' in item:
            specifier = item.split(' ', 1)[1]
        language = getattr(cls, name)
        if not versions:
            return language()
        return limit_versions(language(), specifier)
...
def limit_versions(language, specifier):
    """
    Limits the versions of a `Language` instance. It supports limiting by using
    operators or specifying versions:

    >>> Language['Python ~= 3']
    Python 3.3, 3.4, 3.5, 3.6
    >>> Language['Python 3.5, 3.6, 3.7']
    Traceback (most recent call last):
    ...
    ValueError: 3.7 version does not exist
    >>> Language['py 2']
    Python 2.7
    >>> Language['py < 2']
    Traceback (most recent call last):
    ...
    :param language:
        A `Language` instance.
    :param specifier:
        The specifier to use for the limiting.
    :return:
        A new `Language` instance with limited versions.
    :raises ValueError:
        If no version is left anymore.
    """
    if not specifier[0].isdigit():
        specs = SpecifierSet(specifier + '.0')
        versions = [version for version in language.versions
                    if str(version) in specs]
    else:
        specifiers = specifier.split(', ')
        versions = []
        for specifier in specifiers:
            specs = SpecifierSet('~=' + specifier + '.0')
            version = [version for version in language.versions if str(
                version) in specs]
            if len(specifiers) == 1:
                versions = version
            elif version:
                ...
                raise ValueError(specifier + ' version does not exist')
    if not versions:
